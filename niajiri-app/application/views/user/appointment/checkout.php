...
$mobile_no = $this->db->select('*')->
			from('book_service bs')->
        	join('users u', 'u.id = bs.user_id', 'LEFT')->
        	where("bs.id", $bookings['id'])->
        	get()->row_array();
        ...
        <div class="row">
				<div class="col-md-8">
					...
					<div class="mt-3">
						...
						<?php } if(!empty($cod_option_status)) { ?>
						<div class="form-check">
				...
				<div class="col-md-4">
					...
					<li class="list-group-item d-flex justify-content-between">
					  ...
					  <strong><span><?php echo currency_conversion($user_currency_code); ?></span><span><?php  echo $total_amt;?></span></strong>
					</li>
					...
					<li class="list-group-item d-flex justify-content-between">
					  <span><?php echo (!empty($user_language[$user_selected]['lg_Total_Payable'])) ? $user_language[$user_selected]['lg_Total_Payable'] : $default_language['en']['lg_Total_Payable']; ?> </span>
					  <strong><span><?php echo currency_conversion($user_currency_code); ?></span><span id="total_pay"><?php  echo $total_payable;?></span></strong>
					</li>
            ...
            <form name="frm_paypal_detail" id="frm_paypal_detail" target="_blank" action="<?php echo$form_url?>" method="POST">
				...
				<input type='hidden' name='amount' value='<?php echo $total_payable; ?>'> 
			...
			<input type="hidden" name="cmd" value="_xclick">  
				...
				<?php //$form_url_escrow='https://www.escrow-sandbox.com/checkout'; ?>
            	<!-- <form name="escrow_payment_detail" id="escrow_payment_detail" target="_blank" action="<?php echo$form_url_escrow?>" method="POST">			
					<input type="hidden" name="type" value="domain_name">
					<input type="hidden" name="non_initiator_email" value="maripriya.mathiyalagan@dreamguystech.com">
					<input type="hidden" name="non_initiator_id" value="2874638">
					<input type="hidden" name="non_initiator_role" value="seller">
					<input type="hidden" name="title" value="Test">
					<input type="hidden" name="currency" value="USD">
					<input type="hidden" name="domain" value="https://www.escrow-sandbox.com/checkout">
					<input type="hidden" name="price" value="1000">
					<input type="hidden" name="concierge" value="false">
					<input type="hidden" name="with_content" value="false">
					<input type="hidden" name="inspection_period" value="8">
					<input type="hidden" name="fee_payer" value="seller">
					<input type="hidden" name="return_url" value="">
					<input type="hidden" name="button_types" value="both">
					<input type="hidden" name="auto_accept" value="10">
					<input type="hidden" name="auto_reject" value="0">
					<input type="hidden" name="item_key" value="0">
				</form>  -->
				<!-- Escrow -->
				...
				<div class="modal" id="mpesa_paymodal">
					<div class="modal-dialog">
						<div class="modal-content">
							<div class="modal-header">M-PESA PAYMENT
							</div>
							<button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button>
							<div class="modal-body">
							<form action="#" method='post' id="mpesa_payment_submit" name="mpesa_payment_submit">
								<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" />
								<div class="form-group">
									<input type="text" class="form-control form-control-lg" placeholder="Mobile Number" name='mob_no' id='mob_no' value="<?=$mobile_no['country_code'].$mobile_no['mobileno']?>" readonly>
										<!-- <input type="text" class="form-control form-control-lg" placeholder="Mobile Number without country code" name='mob_no' id='mob_no' 
										 value="<?=$mobile_no['mobileno']?>" > --> 
								</div>
								<div class="form-group">
									<input type="text" class="form-control form-control-lg" placeholder="Amount" name='amount' id='amount' value="<?=get_gigs_currency($total_payable, $user_currency_code, $user_currency_code);?>" readonly>
								</div>

								<div class="form-group">
									<input type="text" class="form-control form-control-lg" placeholder="Ref ID" name="ref_id" id="ref_id" value="174379">
								</div>

								<input type="hidden" class="form-control form-control-lg" placeholder="" name='user_currency' id='user_currency' value="<?=$user_currency_code?>">

								<input type='hidden' name='mpesa_booking_id' id="mpesa_booking_id" value="<?php echo $bookings['id']; ?>">  
								<div class="text-center">
									<input type="submit" name="mpesa_submit" value="PAY" id="mpesa_submit_id">
								</div>
