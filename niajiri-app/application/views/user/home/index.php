		...
		<div class="slider-path">
			<div class="owl-carousel owl-theme owl-carousel" id="category-slider">
				<?php foreach ($categories as $category) {
					...
					$this->db->where('status', 1);
					$cat_services = $this->db->where('category', $category['id'])->count_all_results('services s');
				?>
                ...
                <div class="slider-service">
						...
						<div class="owl-carousel owl-theme owl-carousel sliders-shops owl-loaded owl-drag">
                            <?php foreach ($featured_shops as $frows) {
                                ...
                                $service_count = $this->db->where('shop_id', $frows['id'])->count_all_results('services s');
                                $this->db->where('status', 0);
                                $product_count = $this->db->where('shop_id', $frows['id'])->count_all_results('products');
                ...
                <div class="slider-service">
						...
						<div class="owl-carousel owl-theme owl-carousel sliders-shops owl-loaded owl-drag">
                            <?php foreach ($nearest_shops as $nrows) {
            					...
            					$this->db->where('sd.expiry_date_time>=',date('Y-m-d'));
            					$this->db->where('s.status', 1);
                                $service_count = $this->db->where('shop_id', $nrows['id'])->count_all_results('services s');
                                $this->db->where('status', 0);
                                $product_count = $this->db->where('shop_id', $nrows['id'])->count_all_results('products');
