									...
									if(count($product)>0){
										...
										foreach($product as $row){ 											 
											...
											<tr>
												...
												<td>
													<?php if($row['delivery_status']==8){?>
							<!---------Delivery condition status 5 is changed to status 8 because of  user should satisfy after product delivered , 
							           only then admin will release the payment to provider as per escrow flow--------> 
													<span class="badge bg-success-light">Delivered</span>
													 <?php if($row['cart_flag']==0){?>
													 <span class="badge bg-success-light">Payment <br />Not Released</span>
													 <?php }  if($row['cart_flag']==1){  ?>
													 <span class="badge bg-success-light">Payment <br />Released</span>
													  <?php } ?> 
													<?php } if($row['delivery_status']==6){?>
