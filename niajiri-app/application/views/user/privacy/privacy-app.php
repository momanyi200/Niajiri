		...
		<div class="row">
			<div class="col">
				<div class="breadcrumb-title">
					<h2><?php echo ($privacy_policy[0]['page_title'])?$privacy_policy[0]['page_title']:'privacy Policy.'; ?></h2>
				</div>
			</div>
					...
					<?php if(!empty($result))
						{
							foreach($result as $data){
								if($data['key'] == 'privacy'){
									echo $this->terms = $data['value'];
								}
							}
						}
					?>
					<p><?php echo ($privacy_policy[0]['page_content'])?$privacy_policy[0]['page_content']:'Details Not Found.'; ?></p>
				</div>
			</div>
		</div>
	</div>
