	...
	public function home_post()
	{
		if($this->api_token)
		{
			...
			if ($users_id!=0) 
			{
				...
				// New code to check mobile number is mpesa enabled on 24-12-2022
				$country_code = $user['country_code'];
				$mobile_no=$country_code.$user['mobileno'];

				$mpesa_mobile = $this->api->mpesa_mobile_validation($mobile_no); 
				if($mpesa_mobile>0){
					$mpesa=true;				
				} 
...
// 			}
			...
			$featured_category = $this->api->featured_category_list(4,0,$wcat,$user_data['latitude'], $user_data['longitude']);
			//sub categories
...
// 			}
			...
			// $res['featured_shop_query'] = $this->db->last_query();
            //nearest shops
			...
			if (!empty($featured_category)) 
			{
                ...
                $res['offers'] = $offers;
                $res['cartCount'] = $count;
				$res['mpesa_mobile_enabled']=$mpesa;
                $data = $res;
			}
			else
			{
	...
	public function all_featured_categories_post()
	{
		if($this->api_token)
		{
			...
			//get featured categories
			$wcat = array('status'=>1);
// 			hidden for feature category not showing
// 			if ($gender!='') 
// 			{
...
// 			}
			...
			//$featured_category = $this->api->featured_category_list($user_data['per_page'], $end, $wcat);
				$featured_category = $this->api->featured_category_list($user_data['per_page'], $end, $wcat,$lat,$lang);
			if (!empty($featured_category)) 
			{
		...
		if($this->api_token)
		{
			if (isset($_GET['id']) && !empty($_GET['id'])) 
            {
                ...
                if (!empty($service_details)) 
                {
					...
					$wpservices = array('services.status'=>1);
					$search = array();
				// 	Related Service without curr service
					$where_in = array();
        			$where_in[] = ['type'=>'Out', 'column_name'=>'services.id', 'value'=>$_GET['id']];
					$popular_services = $this->api->popular_services_list(4,0,$wpservices, $search, $where_in, $service_details['service_latitude'], $service_details['service_longitude'], $UserCurrency, $currency_sign, 1, 3);
					if (!empty($popular_services)) {
    ...
    public function update_provider_post() {
        ...
        if ($this->user_id != 0 || ($this->default_token == $this->api_token)) {
            ...
            if (!empty($this->post('user_currency'))) {
                ...
                if ($wallet_history!="" && count($wallet_history) > 0) {
			...
			if (!empty($user_data['subcategory']) && !empty($user_data['account_holder_name']) && !empty($user_data['account_number']) && !empty($user_data['account_iban'])) {
				...
				// empty($user_data['commercial_reg_image'])
				if($validate_commercial_verify == 1 && empty($_FILES['commercial_reg_image']['name'])) {
					$response_code = '500';
    ...
    public function add_service_post() {
        ...
        if ($this->user_id != 0 || ($this->default_token == $this->api_token)) {
            ...
            if (!empty($user_data['shop_id']) && ($user_data['staff_id'] != '') && !empty($user_data['duration']) && !empty($user_data['service_title']) && !empty($user_data['category']) && !empty($user_data['subcategory']) && !empty($user_data['service_amount']) && !empty($user_data['service_offered']) && !empty($user_data['about'])) {
                ...
                $inputs['service_image'] = (!empty($service_image) && count($service_image)>0) ? $service_image[0] : "";
				
				$service = implode(',', array($this->post('service_offered')));
    ...
    public function update_user_post()
    {
    	if ($this->users_id != 0 || ($this->default_token == $this->api_token)) 
    	{
    		...
    		if ($user_data['user_currency']!='') 
    		{
                ...
                $wallet_history = $this->Stripe_model->get_wallet_history_info($token, $currency);
                $credit = $debit = 0;
                if (isset($wallet_history) && count($wallet_history) > 0) 
                {
                    foreach ($wallet_history as $key => $value) {
                ...
                if ($result) {
                    ...
                    if (!empty($result)) {
                        ...
                        foreach ($result as $details) {
							...
							/***************** Newly written code on 21-12-2022 - To check the provider payment released/deposited *********************/
							$res['deposit_flag'] = $details['deposit_flag'];
							if( $res['status']==6 && $res['deposit_flag']==0) {
								$res['provider_payment']='Payment Not Released'; //This means admin not released the service booking payment to provider							
							}
							else if( $res['status']==6 && $res['deposit_flag']==1) {
								$res['provider_payment']='Payment Released'; //This means admin released the service booking payment to provider							
							}
							else{
								$res['provider_payment']='Service is in Process';
							}
            ...
            if ($user_data['type'] == 1) {
                ...
                if ($result) {
                    ...
                    if ($booking_status['status'] == '1' && $user_data['status'] == '1') {
                        ...
                        if ($result) {
                            /* provider completed */
                            $this->send_push_notification($token, $user_data['id'], 2, ' Have Completed Their Service');
...
//                            }
    ...
    public function send_push_notification($token, $service_id, $type, $msg = '') {
     ...
     public function send_push_notification1_get() {
        
       $token = '1N486h3XEZSj2vh';
       $service_id='217'; 
       $type='2'; 
        ...
        public function send_push_notification_mpesa_pay($token,$id, $type, $msg = '',$user_id,$pay_for) {
        ...
        if (!empty($user_id)) {
			...
			if($pay_for=='buyProduct'){
				$order_id = $id; // Order ID				
				$data = $this->api->get_order_info($order_id);
				$title = 'Order Code-'.$data['order_code'].' Payment Status';
			}
			else if($pay_for=='subscription_fee'){
				$subscription_fee_id = $id; // Subscription Fees ID
				$data = $this->api->get_subscribeFee_info($subscription_fee_id);
				$title = 'Subscription-'.$data['subscription_name'].' Payment Status';
			}
			else if($pay_for=='addShop'){
			  $title = "New Shop-Payment Status";
 			}
    ...
    public function language_list_get() {
        ...
        if (!empty($result)) {
            ...
            $response_code = '404';
            $status = FALSE;
            $response_message = 'Your Language is not found';
            $data = [];
    ...
    public function language_post() {
        ...
        if (!empty($user_data['language'])) {
            ...
            if (!empty($result)) {
                ...
                $response_message = 'No language were found';
                $data = [];
    ...
    public function currency_list_get() {
        ...
        if (!empty($result)) {
           ...
           // $response_message = $this->language_content['lg_success'];
            ...
            $response_code = '404';
            //$response_message = $this->language_content['lg_no_language_were_found'];
            $response_message = 'No currency found';
            $data = [];
...
//                                    print_r($amount);exit;
    ...
    public function test_user_product_list_post()
    {
    	if ($this->users_id != 0 || ($this->default_token == $this->api_token)) 
    	{
    		$user_data = $this->post();
			...
			$total = $this->api->test_user_product_list('','', $where, $search, 'count', $UserCurrency, $currency_sign);
    		
            $result = $this->api->test_user_product_list($user_data['per_page'], $end, $where, $search, 'list', $UserCurrency, $currency_sign);
		...
		if ($this->users_id != 0 || ($this->default_token == $this->api_token)) {
            ...
            if (!empty($ser)) {
				...
				// 	$addiamt = array_sum($addiseramt);
					$addiserids = array($this->post('addiservice_id')); 
					$addiseramt = array($this->post('addiservice_amount')); 
					$inputs['additional_services'] = implode(',', $addiserids);
					$addiamt = array_sum($addiseramt);
				}
...
/*apns push notification*/
	...
	/*MPESA Booking service payment */
    public function booking_service_mpesa_payment_post(){
        if ($this->users_id != 0 || ($this->default_token == $this->api_token)) 
    	{
				...
				$mobile_no = $country_code.$post['mobile_no'];
				$booking_id = $post['booking_id'];
				$result=$this->mpesa->checkout($mobile_no,$post['amount'],$post['ref_id'],"book_service",$TransactionType);
				 if($result['ResponseCode']==0) { 
				    $StkPushResponse['MerchantRequestID'] = $result['MerchantRequestID'];  
					$StkPushResponse['CheckoutRequestID'] = $result['CheckoutRequestID'];
					$StkPushResponse['ResponseCode'] = $result['ResponseCode'];	
					$StkPushResponse['ResponseDescription'] = $result['ResponseDescription'];	
					$StkPushResponse['CustomerMessage'] = $result['CustomerMessage'];	
					$StkPushResponse['mpesa_stkpush_created_on'] = date('Y-m-d');	
					$StkPushResponse['cod'] = '2';	
					$this->db->where('id',$booking_id); 					
					$this->db->update('book_service',$StkPushResponse);
					$affected = $this->db->affected_rows(); 
					if($affected>0) { 
						//echo json_encode(['success' => true, 'msg' => $result['ResponseDescription'], 'title' =>'M-PESA Payment', 'status' => 1]);
						
						$response_code = $result['ResponseCode'];
                     	$response_message = $result['ResponseDescription'];
						$StkPushResponse = $result;
 					}
					else{
						//echo json_encode(['success' => false, 'msg' => 'Unable to initiate your order', 'title' =>'M-PESA Payment', 'status' => 2]);
						$response_code = $result['ResponseCode'];
                   		$response_message = 'Unable to initiate your order';
						$StkPushResponse= $result;
						}
				  }	
				 else{
					...
					//echo json_encode(['success' => false, 'msg' => $errmsg, 'title' =>'M-PESA Payment', 'status' => 2]);
					$response_code = $StkPushResponse['ResponseCode'];
                   	$response_message = $errmsg;
					$StkPushResponse= array();

				 }	
				 $this->send_push_notification($this->api_token,$booking_id,2,$response_message);
				  
               	 $result = $this->data_format($response_code, $response_message, $StkPushResponse);
            	 $this->response($result, REST_Controller::HTTP_OK);	
	...
	// M-PESA CustomerBuyGoodsOnline Payments
	public function buyProduct_mpesa_payment_post(){
        ...
        if ($this->users_id != 0 || ($this->default_token == $this->api_token)) 
    	{
                ...
                $result=$this->mpesa->checkout($mobile_no,$post['amount'],$post['ref_id'],"Buying Goods",$TransactionType);
				$order_id = $post['order_id'];
				 if($result['ResponseCode']==0) { 
					$StkPushResponse['Mpesa_MerchantRequestID'] = $result['MerchantRequestID'];
					$StkPushResponse['Mpesa_CheckoutRequestID'] = $result['CheckoutRequestID'];
					$StkPushResponse['Mpesa_ResponseCode'] = $result['ResponseCode'];	
					$StkPushResponse['Mpesa_ResponseDescription'] = $result['ResponseDescription'];	
					$StkPushResponse['Mpesa_CustomerMessage'] = $result['CustomerMessage'];	
					$StkPushResponse['payment_type'] = 'MPESA-Mobile';
 					$StkPushResponse['payment_gway'] = 'MPESA';
					$StkPushResponse['transaction_id'] = $result['CheckoutRequestID'];
					//$StkPushResponse['mpesa_stkpush_created_on'] = date('Y-m-d');	
					$this->db->where("id",$order_id); 
					$this->db->update('product_order', $StkPushResponse);
					$affected = $this->db->affected_rows(); 
					if($affected>0) { 
						//echo json_encode(['success' => true, 'msg' => $result['ResponseDescription'], 'title' =>'M-PESA Payment', 'status' => 1]);
						$response_code = $result['ResponseCode'];
					...
					$response_code = $StkPushResponse['ResponseCode'];
                   	$response_message = $errmsg;
					$StkPushResponse= $result['errorMessage'];

				 }	 
				 $this->send_push_notification_mpesa_pay($this->api_token,$order_id,2,$response_message,$this->users_id,'buyProduct');
               	 $result = $this->data_format($response_code, $response_message, $StkPushResponse);
            	 $this->response($result, REST_Controller::HTTP_OK);	
			...
			/*MPESA Provider Subscription payment */
    public function provider_subscription_mpesa_payment_post(){
        if ($this->user_id != 0 || ($this->default_token == $this->api_token)) 
    	{
 				$post=$this->input->post();
 				$inputs = array();
				...
				$country_code = $post['country_code'];
				$mobile_no = $country_code.$post['mobile_no'];
				$subscription_id = $post['subscription_fee_id'];
				
				$result=$this->mpesa->checkout($mobile_no,$post['amount'],"174379","subscription",$TransactionType);
				 if($result['ResponseCode']==0) { 
 				    $StkPushResponse['MerchantRequestID'] = $result['MerchantRequestID'];  
					$StkPushResponse['CheckoutRequestID'] = $result['CheckoutRequestID'];
					$StkPushResponse['subscription_date'] =  date('Y-m-d H:i:s');
					$StkPushResponse['subscriber_id'] = $this->user_id;	
					$StkPushResponse['subscription_id'] = $subscription_id;	 
					$StkPushResponse['payment_details'] = $result;
					$inputs['subscription_id'] = $subscription_id;
					$inputs['user_id'] = $this->user_id;
					$inputs['tokenid']=$result['MerchantRequestID'];
					$inputs['payment_details']=$result['ResponseDescription'];
					$inputs['paytype'] = 'MPESA-Mobile';
 					$affected = $this->api->mpesa_subscription_pay($inputs);
					if($affected==true) { 
						//echo json_encode(['success' => true, 'msg' => $result['ResponseDescription'], 'title' =>'M-PESA Payment', 'status' => 1]);
						$response_code = $result['ResponseCode'];
                     	$response_message = $result['ResponseDescription'];
					...
					//echo json_encode(['success' => false, 'msg' => $errmsg, 'title' =>'M-PESA Payment', 'status' => 2]);
					$this->send_push_notification_mpesa_pay($this->api_token,$subscription_id,1,$response_message,$this->user_id,'subscription_fee');
					$response_code = $StkPushResponse['ResponseCode'];
                   	$response_message = $errmsg;
					$StkPushResponse= array();
    ...
    /*end*/
		...
		/*MPESA Provider add shop mpesa payment */
 public function provider_add_shop_mpesa_payment_old_post(){
        if ($this->user_id != 0 || ($this->default_token == $this->api_token)) 
    	{
 				$post=$this->input->post();
 				$inputs = array();
                ...
                $post = $this->post(); */
				$country_code = $post['country_code'];
				$shop_fee = $post['shop_fee'];
				$mobile_no = $country_code.$post['mobile_no'];
				$ref_id = "174379";
				
				$result=$this->mpesa->checkout($mobile_no,$post['shop_fee'],$ref_id,"Provider Add Shop Payment",$TransactionType);
				 if($result['ResponseCode']==0) { 
 				    $StkPushResponse['MerchantRequestID'] = $result['MerchantRequestID'];  
					$StkPushResponse['CheckoutRequestID'] = $result['CheckoutRequestID'];
					$StkPushResponse['Mpesa_ResponseCode'] = $result['ResponseCode'];	
    ...
    /*end*/
	/*MPESA Provider add shop mpesa payment */
    public function provider_add_shop_mpesa_payment_post(){
        if ($this->user_id != 0 || ($this->default_token == $this->api_token)) 
    	{
 				$post=$this->input->post();
 				$inputs = array();
				...
				$result=$this->mpesa->checkout($mobile_no,$post['shop_fee'],$ref_id,"add_shop",$TransactionType);
				 ...
				 if($result['ResponseCode']==0) { 
					...
					$details['type']=1;  // For provider - type 1 
					$details['service_subscription_id'] = 0; 
					$details['token'] = $token;
					$details['user_provider_id'] = $user_id;
					$details['currency_code'] = $user_currency_code;
					$details['amount'] = $shop_fee;
					$details['reason'] = "Add Shop";
					$details['transaction_id'] = $result['CheckoutRequestID'];
					$details['created_at'] = date('Y-m-d H:i:s'); 
					$details['book_id'] = 0;
					$details['total_amount'] = $shop_fee;
					$details['paytype'] = 'mpesa';
					$details['response'] = $result['ResponseDescription'];
					$details['status'] = 2;
					$this->db->insert('moyasar_table', $details); 
 					$affected = $this->db->affected_rows(); 
					if($affected>0) { 
				 ...
				 $this->send_push_notification_mpesa_pay($this->api_token,0,1,$response_message,$this->user_id,'addShop');
               	 $result = $this->data_format($response_code, $response_message, $StkPushResponse);
            	 $this->response($result, REST_Controller::HTTP_OK);	
    ...
    /*end*/
	 public function booking_service_mpesa_payment_status_post(){
        if ($this->users_id != 0 || ($this->default_token == $this->api_token)) 
    	{
 				$post=$this->input->post();
				$booking_id= $post['booking_id'];
  				$book_id_pay_status=$this->db->query("select * from book_service WHERE id = $booking_id");
 				$count=$book_id_pay_status->num_rows(); 
					if($count>0) {
  					   $book_id_pay_status= $this->db->query("select * from book_service WHERE id=$booking_id")->result_array();
 					   $status=$book_id_pay_status[0]['status'];   
						 
						if($status=='2') {   
						$response_code = '200';
						$StkPushResponse = "success";
						$response_message ='Your Payment for Service Booking is Success';
 						} 
						else if($status=='8'){ 
							$response_code = '201';
 							$StkPushResponse = "pending" ;
							$response_message ='Your Payment for Service Booking is Pending';
						}  
						else { 
							$response_code = '201';
							if($payment_status!='') { 
							$StkPushResponse=$payment_status; 
							$response_message ='Your Payment for Service Booking is '.$payment_status;
							} else{
 							$StkPushResponse = "Failed/Cancelled" ;
							$response_message ='Your Payment for Service Booking is Failed/Cancelled';
							}
						} 
						//echo json_encode(['success' => true, 'msg' => $result['ResponseDescription'], 'title' =>'M-PESA Payment', 'status' => 1]);
						...
						//echo json_encode(['success' => false, 'msg' => 'Unable to initiate your order', 'title' =>'M-PESA Payment', 'status' => 2]);
						$response_code = '202'; // Failed
                     	$response_message ='Unable to reach the server';
	  ...
	  public function buyProduct_mpesa_payment_status_post(){
        if ($this->users_id != 0 || ($this->default_token == $this->api_token)) 
    	{
 				$post=$this->input->post();
				$order_id= $post['order_id'];
				
				    $prd_order_id_pay_status= $this->db->where('id',$order_id)->select('*')->get('product_order')->result_array(); 					
 					$affected = $this->db->affected_rows(); 
					if($affected>0) {
					    $payment_status =  $prd_order_id_pay_status[0]['payment_status'];
					    if($prd_order_id_pay_status[0]['status']==1) {   
						$StkPushResponse='success';	
						$response_code = '200';	
						$response_message = 'Your Payment is success for the Order Code '.$prd_order_id_pay_status[0]['order_code'];				 
						} 
						else{ 
							$StkPushResponse='Pending';
							$response_code = '201';
							$response_message = 'Your Payment is pending for the Order Code '.$prd_order_id_pay_status[0]['order_code'];	
						} 
						//echo json_encode(['success' => true, 'msg' => $result['ResponseDescription'], 'title' =>'M-PESA Payment', 'status' => 1]);
  					}
					else{
	 ...
	 public function provider_subscription_mpesa_payment_status_post(){
        if ($this->user_id != 0 || ($this->default_token == $this->api_token)) 
    	{
 				$post=$this->input->post();
				$subscription_fee_id= $post['subscription_id'];
				    $prd_order_id_pay_status= $this->db->where('id',$subscription_fee_id)->select('*')->get('subscription_details')->result_array(); 					
 					$affected = $this->db->affected_rows(); 
 					if($affected>0) {
					    if($prd_order_id_pay_status[0]['paid_status']==1) {   
						$payment_status='success';						 
						$response_code = '200';
						$response_message = 'Your Paymnet for subscription  is Successful';
						$StkPushResponse = $payment_status;
						} 
						else{ 
							$payment_status='pending';
							$response_code = '201';
							$response_message = 'Your Payment for subscription is Pending';
						    $StkPushResponse = $payment_status;
						} 
						//echo json_encode(['success' => true, 'msg' => $result['ResponseDescription'], 'title' =>'M-PESA Payment', 'status' => 1]);
				...
				$this->send_push_notification_mpesa_pay($this->api_token,$subscription_fee_id,2,$response_message,$this->user_id,'subscription_fee');
				$result = $this->data_format($response_code, $response_message, $StkPushResponse);
            	 $this->response($result, REST_Controller::HTTP_OK);	
	    ...
	    public function provider_add_shop_mpesa_payment_status_post(){
        if ($this->user_id != 0 || ($this->default_token == $this->api_token)) 
    	{
	 			$post=$this->input->post();
				$user_id= $post['user_id'];
				$transaction_id=$post['CheckoutRequestID'];
    				$query=$this->db->query("select * from moyasar_table WHERE user_provider_id=".$user_id." and type=1 and transaction_id='".$transaction_id."'");
 				$affected= $query->num_rows(); 
 					$affected = $this->db->affected_rows(); 
 					if($affected>0) {
					   $prd_order_id_pay_status=$query->result_array();
					    if($prd_order_id_pay_status[0]['status']==1) {   
						$payment_status='success';						 
						$response_code = '200';
						$response_message = "Your Payment for \'Adding Shop\' is Successful";
						$StkPushResponse = $payment_status;
						} 
						else{ 
							$payment_status='pending';
							$response_code = '201';
							$response_message = 'Your Payment for \'Adding Shop\' is Pending';
						$StkPushResponse = $payment_status;
						} 
						//echo json_encode(['success' => true, 'msg' => $result['ResponseDescription'], 'title' =>'M-PESA Payment', 'status' => 1]);
  					}
	   			...
	   			$this->send_push_notification_mpesa_pay($this->api_token,$subscription_fee_id,1,$response_message,$this->user_id,'addShop');
				$result = $this->data_format($response_code, $response_message, $StkPushResponse);
            	 $this->response($result, REST_Controller::HTTP_OK);	
	 ...
	 public function user_order_list_changeSatisfy_post()
    {
		  if ($this->users_id != 0 || ($this->default_token == $this->api_token)) 
    	{
			$user_inp = $this->input->post();
			$order_id = $user_inp['order_id'];
			$product_id = $user_inp['product_id'];
			//order details
				$this->db->where(array('product_cart.order_id'=>$order_id,'product_cart.product_id'=>$product_id));
				$this->db->update('product_cart', array('delivery_status'=>8)); 
  			 	$affected= $this->db->affected_rows();   
				// Here delivery status 8 means , User Satisfied in the product delivery, Only then admin will release the payment to provider
				if($affected>0) {  
				     $response_code='200';
					 $response_message ='Updated as Satisfied';
					 $StkPushResponse = 'success'; 				
					 }
				else{
				    $response_code='201';
					 $response_message ='Not Updated as Satisfied';
					$StkPushResponse = 'failed';
				}
	 ...
	 public function mpesa_mobile_validation_post(){
	           
	    if ($this->users_id != 0 || ($this->default_token == $this->api_token)) 
    	{        
	            $mobile_no = '254'.$this->input->post('mobile_no');
 				$TransactionType='CustomerBuyGoodsOnline';
				$result=$this->mpesa->checkout($mobile_no,'1',"123456","Do not pay. This is just for checking MPESA enabled mobile.. Thanks..",$TransactionType);
				 if($result['errorCode']=='400.002.02') {
				   ...
				   $errors['mpesastk'] = $result['errorMessage'];
					foreach($errors as $error) {
				 	...
				 	$response_code='500';
					 $response_message ='Not a MPESA Enabled Mobile Number';
					 $StkPushResponse = $result['errorMessage']; 	
				 }
				 else{
 					...
 					$response_code='200';
					 $response_message ='This is MPESA enabled Mobile Number';
					 $StkPushResponse = $result; 				 
				 }
		 $result = $this->data_format($response_code, $response_message, $StkPushResponse);
            	 $this->response($result, REST_Controller::HTTP_OK);	
