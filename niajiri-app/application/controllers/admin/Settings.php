...
$live_braintree_merchant = $live_braintree_key = $live_braintree_publickey = $live_braintree_privatekey = $live_paypal_appid = $live_paypal_appkey = '';
  ...
  $data['live_paypal_appkey']    = $live_paypal_appkey;
    ...
    public function mpesa_payment_gateway() {
        $this->common_model->checkAdminUserPermission(14);  
        $id = settingValue('mpesa_option');
        if (!empty($id)) {
            $this->data['list'] = $this->admin->edit_mpesa_gateway($id);
        } else {
            $this->data['list'] = [];
            $this->data['list']['id'] = '';
        ...
        $this->data['page'] = 'mpesa_payment_gateway';
        $this->load->vars($this->data);
        $this->load->view($this->data['theme'] . '/template');
    }
    ...
    public function mpesa_edit($id = NULL) {
        $this->common_model->checkAdminUserPermission(14);
        if ($this->input->post('form_submit')) {
            if ($_POST['gateway_type'] == "sandbox") {
                $id = 1;
            ...
            $show = !empty($this->input->post('mpesa_show'))?$id :0;

            
            $data['gateway_type'] = $this->input->post('gateway_type');
            $data['consumer_key'] = $this->input->post('consumer_key');
            $data['consumer_secret'] = $this->input->post('consumer_secret');
            $data['token_link'] = $this->input->post('token_link');
            $data['checkout_processlink'] = $this->input->post('checkout_process_link');
            $data['checkout_shortcode'] = $this->input->post('checkout_shortcode');
            $data['checkout_passkey'] = $this->input->post('checkout_passkey');
            $data['checkout_rcallbackurl'] = $this->input->post('checkout_rcallbackurl');
            $data['checkout_querylink'] = $this->input->post('checkout_querylink');
            $data['c2b_registerUrl'] = $this->input->post('c2b_registerUrl');
            $data['c2b_shortcode'] = $this->input->post('c2b_shortcode');
            $data['c2b_confirmationUrl'] = $this->input->post('c2b_confirmationUrl');
            $data['c2b_validationUrl'] = $this->input->post('c2b_validationUrl');
            $data['c2b_transactionUrl'] = $this->input->post('c2b_transactionUrl');
            $data['status'] = !empty($this->input->post('mpesa_show'))?$this->input->post('mpesa_show'):0;

            $this->db->where('id', $id);

            if ($this->db->update('mpesa_payment_gateway', $data)) {

                if ($this->input->post('gateway_type') == 'sandbox') {
                    $datass['gateway_type'] = $this->input->post('gateway_type');
                    $datass['consumer_key'] = $this->input->post('consumer_key');
                    $datass['consumer_secret'] = $this->input->post('consumer_secret');
                    $datass['token_link'] = $this->input->post('token_link');
                    $datass['checkout_processlink'] = $this->input->post('checkout_process_link');
                    $datass['checkout_shortcode'] = $this->input->post('checkout_shortcode');
                    $datass['checkout_passkey'] = $this->input->post('checkout_passkey');
                    $datass['checkout_rcallbackurl'] = $this->input->post('checkout_rcallbackurl');
                    $datass['checkout_querylink'] = $this->input->post('checkout_querylink');
                    $datass['c2b_registerUrl'] = $this->input->post('c2b_registerUrl');
                    $datass['c2b_shortcode'] = $this->input->post('c2b_shortcode');
                    $datass['c2b_confirmationUrl'] = $this->input->post('c2b_confirmationUrl');
                    $datass['c2b_validationUrl'] = $this->input->post('c2b_validationUrl');
                    $datass['c2b_transactionUrl'] = $this->input->post('c2b_transactionUrl');
                } else {
                    $datass['gateway_type'] = $this->input->post('gateway_type');
                    $datass['consumer_key'] = $this->input->post('consumer_key');
                    $datass['consumer_secret'] = $this->input->post('consumer_secret');
                ...
                $mpesa_option = settingValue('mpesa_option');

                if (isset($mpesa_option)) {
                    $this->db->where('key', 'mpesa_option')->update('system_settings', ['value' => $show]);
                    
                } else {
                    $this->db->insert('system_settings', ['key' => 'mpesa_option', 'value' => $show]);
                    // echo "kjkl";
                }
                // print_r($mpesa_option);exit();
                foreach ($datass AS $key => $val) {
                    $this->db->where('key', $key);
                    $this->db->delete('system_settings');
                    $table_data['key'] = $key;
            ...
            $this->session->set_flashdata('success_message', $message);
            redirect(base_url() . 'admin/mpesa-payment-gateway');
        }
	...
	public function homeservice_settings(){ 
		...
		$data['book_service_fee'] = !empty($this->input->post('book_service_fee_val'))?$this->input->post('book_service_fee_val'):0;
		$data['buy_product_fee'] = !empty($this->input->post('buy_product_fee_val'))?$this->input->post('buy_product_fee_val'):0;
...
public function generalSetting() {
    ...
    public function add_provider_session() {
      ...
      if($id!="" && $id>0){$this->session->set_userdata('provider_id',$id); }
