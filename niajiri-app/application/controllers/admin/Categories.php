...
class Categories extends CI_Controller {
    ...
    public function add_categories() {
        ...
        if ($this->input->post('form_submit')) {
            ...
            if (isset($_FILES) && isset($_FILES['category_image']['name']) && !empty($_FILES['category_image']['name'])) {
                if(!is_dir('uploads/category_images')) {
                    mkdir('./uploads/category_images/', 0777, TRUE);
                }
			...
			$table_data['category_slug'] = strtolower(trim(preg_replace('/[\s-]+/', '-', preg_replace('/[^A-Za-z0-9-]+/', '-', preg_replace('/[&]/', 'and', preg_replace('/[\']/', '', iconv('UTF-8', 'ASCII//TRANSLIT', $this->input->post('category_name')))))), '-'));
    ...
    public function add_subcategories() {
        ...
        if ($this->input->post('form_submit')) {
            ...
            if (isset($_FILES) && isset($_FILES['subcategory_image']['name']) && !empty($_FILES['subcategory_image']['name'])) {
                 if(!is_dir('uploads/subcategory_images')) {
                    mkdir('./uploads/subcategory_images/', 0777, TRUE);
                }
			...
			$table_data['subcategory_slug'] = strtolower(trim(preg_replace('/[\s-]+/', '-', preg_replace('/[^A-Za-z0-9-]+/', '-', preg_replace('/[&]/', 'and', preg_replace('/[\']/', '', iconv('UTF-8', 'ASCII//TRANSLIT', $this->input->post('subcategory_name')))))), '-'));
    ...
    public function delete_subcategory() {
		...
		if ($this->db->update('subcategories', $table_data)) {
			...
			$this->db->update('services', $table_data);			
			/* New Code - Delete Sub Sub Category */
			$query1 = $this->db->get('sub_subcategories');
			$result = $query1->result_array(); 			
			for($i=0;$i<$query1->num_rows();$i++) {
				$this->db->where('subcategory', $id);
				$this->db->update('sub_subcategories', $table_data);
				$this->db->update('services', $table_data,array('sub_subcategory' => $result[$i]['id']));
			}			
			/* New Code - Delete Sub Sub Category */
	...
	public function load_subcategory(){
		...
		$allval = '<option value="">Select Sub Category</option>';
		if($val != ''){
			$subcategories = $this->db->query(" SELECT * FROM `subcategories` WHERE `status` = 1 and `category` = ".$val)->result_array();
		
			if(count($subcategories) > 0) {
				foreach ($subcategories as $rows) { 
					$allval .= '<option value="'.$rows['id'].'">'.$rows['subcategory_name'].'</option>';
				}
			} 
		}
		echo $allval;
    ...
    //Update Category as Featured or not 
