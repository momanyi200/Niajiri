...
class Products extends CI_Controller {
   ...
   public function __construct() {
	...
	// M-PESA Payments
	public function buyProduct_mpesa_payment(){
                ...
                $result=$this->mpesa->checkout($mobile_no,$post['amount'],$ref_id,"Buying Goods",$TransactionType);
				$order_id = $post['order_id'];
                // print_r($result);
                if($result && isset($result['errorCode']))
                {
                    echo json_encode(['success' => false, 'msg' => $result['errorMessage'], 'title' =>'M-PESA Payment', 'status' => 2]);
                }
					...
					$StkPushResponse['billing_details_id'] = $billing_id;
					$StkPushResponse['address_type'] = $address_type;
					...
					$this->db->update('user_billing_details', ['default_address'=>1]);
					  ...
					  $ins_data = ['product_order_id'=>$order_id, 'user_id'=>$userId, 'payment_gway'=>'mpesa', 'transaction_id'=>$result['CheckoutRequestID'], 'payment_status'=>'pending', 'created_on'=>date("Y-m-d H:i:s")];
        $this->db->insert('product_order_log', $ins_data);
					
					if($affected>0) { 
                        $this->db->where("status",'0'); 
                        $this->db->where("order_id",$order_id); 
                        $this->db->update('product_cart', array("status"=>1));
	 ...
	 public function prd_mpesa_call_back_old()
    {
        // echo "Return";
           ...
           // $callbackData=json_decode($callbackJSONData);
			...
			$payment_transaction_date =  date("Y-m-d H:i:s"); //Date and time
			...
			$TransactionDate	 =  $res['Body']['stkCallback']['TransactionDate']; 
			 if($ResultCode == 0){ 
			...
			$this->db->where(array('transaction_id'=>$CheckoutRequestID,'payment_gway'=>'MPESA')); 
            $this->db->update('product_order', array('payment_status' => $payment_status,'status'=>$status));
        	
    }
	public function prd_mpesa_call_back()
    {
        // echo "Return";
           ...
           // $callbackData=json_decode($callbackJSONData);
			...
			$this->db->where('transaction_id', $CheckoutRequestID);
 			$query = $this->db->get('product_order')->row_array();
			$order_id   = $query['order_id'];
			
       		$this->db->where('order_id', $order_id);
        	$this->db->update('product_cart', array('status'=>$status) );  
      		$this->db->where('transaction_id', $CheckoutRequestID);
       		$this->db->update('product_order_log', array('payment_status' => $payment_status));  
			$this->db->where(array('transaction_id'=>$CheckoutRequestID,'payment_gway'=>'MPESA')); 
            $this->db->update('product_order', array('payment_status' => $payment_status,'status'=>$status));
			  /* if ($gateway!='cod' && $transaction_id!='') 
				{
	...
	public function changeSatisfy()
    {
		$user_inp = $this->input->post();
		$order_id = $user_inp['order_id'];
		$cart_id = $user_inp['cart_id'];
        //order details
        	$this->db->where(array('order_id'=>$order_id,'id'=>$cart_id));
			...
			if($affected>0) {  
				 $result = array('error'=>true, 'msg'=>'Updated as Satisfied');
			}
			else{
				$result = array('error'=>false, 'msg'=>'Not Updated');
			}
			echo json_encode($result); 
