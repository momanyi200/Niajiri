...
{ 
 ...
 { 
  ...
  $bodyid = 1;
	...
	$cname = $this->db->select('category_type')->where('id',$input_data['category'])->get('categories')->row()->category_type;
	 if($cname == '4'){
		$usertype  = 'freelancer';
	 } else {
		 $usertype = 'provider';
...
public function checkforgotmail()
	{ 
		...
		if(!empty($result))
		{
			...
			$chk_forpawd=$this->db->where('user_id',$result['id'])->where('user_type',$user_type)->where('status','1')->select('*')->order_by('id','DESC')->get('forget_password_det')->result_array(); 
			...
			else
			{
			  ...
			  'updated_on'=>date('Y-m-d H:i:s')
				...
				$save_forpwd = $this->admin->update_password_resetlink_data($pwdlink_data,$wherearr);
			}
