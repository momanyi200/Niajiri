...
class Shop extends CI_Controller {
   ...
   public function __construct() {
	...
	public function index()
	{
		...
		$mpesa_option_status = $this->db->where('id',settingValue('mpesa_option'))->get('mpesa_payment_gateway')->row()->status;
   		$this->data['mpesa_option_status']=($mpesa_option_status)?$mpesa_option_status:0;
	...
	// Mpesa Payment initiate
	public function mpesa_shop_payment(){ 	
		
		$status = $this->input->post('status');		
		$token=$this->session->userdata('chat_token');	
		$TransactionType='CustomerBuyGoodsOnline';
		$country_code = '254';
		$mobile_no = $country_code.$this->input->post('mobile_no');
		//$mobile_no = '254723805041';
		$ref_id = $this->input->post('ref_id');
		$shop_fee = $this->input->post('amount')/100;
		$result=$this->mpesa->checkout($mobile_no,$shop_fee,$ref_id,"add_shop",$TransactionType);
		...
		if( $status== 'initiate') {	
			...
			$affected = $this->db->affected_rows(); 
 			if($affected>0){ 
			$this->session->set_flashdata('success_message',  $result['ResponseDescription']);
			
			//$this->send_push_notification($token,$this->session->userdata('id'),1,' -Your Payment is in processing for New Shop Fee');
			echo json_encode(array('success'=>true,'message'=>'Your Payment is in processing for New Shop Fee'));
			}
			
	    }
		else{
				$this->session->set_flashdata('success_message', $this->feemsg);
 			  //$this->send_push_notification($token,$this->session->userdata('id'),1,' - Unable to initiate payment.........');
				echo json_encode(array('success'=>false,'message'=>'Unable to initiate payment'));
			}
		} else{
	    	echo json_encode(array('success'=>false,'message'=>$result['errorMessage']));
			$data=array('error_message'=>$result['errorMessage']);
			$this->session->set_flashdata($data);
		}      
		
	}
	
	// MPESA Add shop payment call back url
	public function addshop_mpesa_call_back()
    {
			...
			$this->db->where(array('transaction_id'=> $CheckoutRequestID)); 
            $this->db->update('moyasar_table', array('status' => $payment_status,'updated_on'=>$payment_transaction_date));
