...
class Appointment extends CI_Controller {
	...
	public function book_appointment() {
		...
		if ($this->session->userdata('usertype') == 'user') {		
			...
			if($covidControl == 1 && $cvaccine == 0){
				...
				echo '<h1 align="center">Permissions Denied - Please Update the Covid Vaccination Status of Yours for Booking</h1>'.$content;die;
			}
		}
	...
	// public function book_mpesa_payment(){
	// 	$Passkey = '';
	//     $Amount= $_POST['amt'];
	//     $BusinessShortCode = '174379';
	//     $PartyA ="+919487221886";
	//     $AccountReference ="Mari";
	...
	//         $ConsumerKey = 'MVZAF2D4NhHxA4w3J112bF4LtdO8XhZz';
	//         $ConsumerSecret = 'ZAlKpDztP5sRDaPH';
	...
	public function book_staffservice() {
		...
		if($book_id > 0){					
			...
			$totalamtval = $this->input->post('total_amt');
			$totalamt = $cal_seramt;
	...
	function check_availability(){
		...
		if (!empty($from_time1)) {
			...
			echo json_encode(['success' => false, 'msg' => $this->nobookmsg,  'title' => $this->BMsg, 'status' => 3]);exit;
		}
	...
	function get_coupon_details(){
		...
		if($coupon['id'] > 0) { 
			...
			//Set Coupon in session for paypal payment
			...
			$licontent = '<div class="text-success" id="cpnli">
              ...
              <span class="text-success">-'.currency_conversion($rec['currency_code']).'<span id="cpn_price">'.$couponPrice.'</span></span>';
	...
	// M-PESA Payments
	public function booking_service_mpesa_payment(){
                
				$post=$this->input->post();
				$booking_id = $post['bookid'];
				$TransactionType='CustomerBuyGoodsOnline';
				$ref_id =174379;
				//$country_code = '254';
				$mobile_no = $this->input->post('mobile_no');
				//echo $post['mobile_no'].'-'.$post['amount'].'-'.$ref_id.'-'."book_service".'-'.$TransactionType; exit; 
                $result=$this->mpesa->checkout($mobile_no,$post['amount'],$ref_id,"book_service",$TransactionType);
				 if($result['ResponseCode']==0) { 
					...
					$StkPushResponse['status'] = '1';
					$StkPushResponse['cod'] = '2';
					$this->db->where("id",$booking_id); 
					$this->db->update('book_service', $StkPushResponse);
					$affected = $this->db->affected_rows(); 
					$msg = $result['ResponseCode'].'-'.$result['ResponseDescription'];
					if($affected>0) { 
						echo json_encode(['success' => true, 'msg' => $msg, 'title' =>'M-PESA Payment', 'status' => 1]);
					}
					else{
						echo json_encode(['success' => false, 'msg' => $result, 'title' =>'M-PESA Payment', 'status' => 2]);
						}
				  }	
				 else{
	 ...
	 public function mpesa_call_back()
    {
        // echo "Return";
           ...
           // $callbackData=json_decode($callbackJSONData);
			...
			$this->db->where(array('MerchantRequestID'=>$MerchantRequestID,'CheckoutRequestID'=> $CheckoutRequestID)); 
            $this->db->update('book_service', array('status'=>$status,'payment_status' => $payment_status,'TransactionDate' => $TransactionDate));
        	
    }
    
    public function mpesa_call_back_service_booking() { 
        ...
        // echo "Return";
           ...
           // $callbackData=json_decode($callbackJSONData);
			...
			$ResultDesc			 = 	$res['Body']['stkCallback']['ResultDesc']; 
			$TransactionDate	 =  $res['Body']['stkCallback']['TransactionDate']; 
 		    $book_details =  $this->db->get_where('book_service', array('MerchantRequestID'=>$MerchantRequestID,'CheckoutRequestID'=> $CheckoutRequestID))->result_array();
			$bookid =  $book_details['id'];
			
			$servicess = $this->db->select('*')->where('id',$bookid)->get('book_service')->row_array();
 			$service_id = $servicess['service_id']; 
			$amt  = $book_details['amount'];
			$inputs = array();		
		...
		if(isset($ResultCode) && $ResultCode==0) {			
			...
			if ($result != '') {	
				...
				$inputs['reason'] = '';
				$inputs['final_amount'] = $amt;
				$inputs['paid_tokenid'] = $CheckoutRequestID;
				$inputs['paytype'] = 'mpesa';
				$inputs['payment_status'] = 'success';
				...
				if($inputs['cod'] == 2){ // not cod					
					...
					$details['amount'] = $servicess['amount'];
					$details['reason'] = "Book Service";
					$details['transaction_id'] = $CheckoutRequestID;
					$details['created_at'] = date('Y-m-d H:i:s'); 
					$details['book_id'] = $result;
					$details['total_amount'] = $totamt;
					$details['type']=$provider_data['type'];
