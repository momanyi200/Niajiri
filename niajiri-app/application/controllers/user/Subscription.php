...
class Subscription extends CI_Controller {
   ...
   public function __construct() {
    ...
    echo json_encode($result);
  	...
  	public function mpesa_subscription_payment(){
    ...
    $user_id = $this->session->userdata('id');
 	$inputs['payment_details']='mpesa';
	$TransactionType='CustomerBuyGoodsOnline';
	$ref_id =174379; 
	//$ref_id = "123456";
		$result=$this->mpesa->checkout($mobile_no,$amount,$ref_id,"subscription",$TransactionType);
				// print_r($result); 
 				 if($result['ResponseCode']==0) { 
					...
					$StkPushResponse['subscriber_id'] = $user_id;	
					$StkPushResponse['subscription_id'] = $sub_id;	 
					...
					$inputs['paytype'] = 'MPESA-Mobile';
 					$affected = $this->subscription->mpesa_subscription_pay($inputs);
					if($affected>0) { 
						//echo json_encode(['success' => true, 'msg' => $result['ResponseDescription'], 'title' =>'M-PESA Payment', 'status' => 1]);
						$data=array('tab_ctrl'=>2,'success_message'=>$result['ResponseDescription']);
                    $this->session->set_flashdata($data);
                   // $token=$this->session->userdata('chat_token');
     		    	//	  $this->send_push_notification($token,$this->session->userdata('id'),1,' have been subscribed'); 
 					  $results= array('msg' =>$result['ResponseDescription']);
					}
					else{
						//echo json_encode(['success' => false, 'msg' => 'Unable to initiate your order', 'title' =>'M-PESA Payment', 'status' => 2]);
						 $data=array('tab_ctrl'=>2,'success_message'=>'Sorry, something went wrong');
                  		 $this->session->set_flashdata($data);
						  $results= array('msg' =>$result);
						}
				  }	
					...
					$results= array('msg' =>'No action for you transction');
				}
  ...
  // MPESA Provider's Subscription payment call back url
	public function subscription_mpesa_callbackurl()
    {
			...
			$this->db->where(array('tokenid'=> $MerchantRequestID)); 
 			$this->db->update('subscription_payment',array('status'=>$payment_status,'payment_details'=>$reason));
