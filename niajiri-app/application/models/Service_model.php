...
class Service_model extends CI_Model
{
	...
	{
		$this->db->select("id,service_image,is_default");
	...
	public function freelancer_subscription_list()
        {
    ...
    public function subscription_details($id)
        ...
        public function get_provider_list(){
            return $this->db->where("status",1)->where('type',1)->get('providers')->result_array();
        ...
        public function get_service_list(){
            return $this->db->where("status !=",0)->get('services')->result_array();
...
/*get Ratings and Reviews*/
    ...
    public function ratingstype_details($id)
        ...
        var $column_order = array(null, 'U.name','U.mobileno','U.email','U.created_at','S.subscription_name','U.country_code');
    ...
    public function get_full_notification($params = array()){
       if(!empty($this->session->userdata('chat_token'))){
        ...
        $this->db->where("notification_table.delete_status != ",1);
	 ...
	 /*freelancer filter*/
        ...
        public function freelancer_filter($username,$email,$from,$to,$subcategory){
          if(!empty($from)) {
          $from_date=date("Y-m-d 00:00:00", strtotime($from));
          }else{
          $from_date='';
	...
	/* Additional Services and Filter */
	public function additional_services_list()
    {
      $this->db->select('A.*,S.service_title');
      $this->db->from('additional_services A');
	...
	public function additional_services_filter($service_id,$service_title,$from,$to){
          if(!empty($from)) {
          $from_date=date("Y-m-d 00:00:00", strtotime($from));
          }else{
          $from_date='';
		...
		$this->db->join('services S','S.id=A.service_id','left');   
          ...
          if(!empty($service_title)){
          $this->db->where('A.id',$service_title);
          }
          if(!empty($service_id)){
          ...
          if(!empty($from_date)){
          $this->db->where('A.created_at >=',$from_date);
          }
          if(!empty($to_date)){
          $this->db->where('A.created_at <=',$to_date);
          }
          $this->db->where('A.status != ',0);	  
          $this->db->where('S.status != ',0);
	...
	public function additional_service_details($id)
	{
		return $this->db->select('*')->where('id',$id)->get('additional_services')->row_array();
	}
	 
	/* Read Category, Sub Category, Sub Sub Category */	
	...
	public function readSubSubcategory($id='')
	{
		$user_id = $this->session->userdata('id');
		$category = $this->db->select('category')->where('id',$user_id)->get('providers')->row()->category;
		$subcategory = $this->db->select('subcategory')->where('id',$user_id)->get('providers')->row()->subcategory;
    ...
    public function adminreadSubcategory()
    {
        return $this->db->get('subcategories')->result_array();
    }
    public function adminreadSubSubcategory($id='')
    {
	...
	{
	     return $this->db->get('sub_subcategories')->result_array();
	}	
	
	public function get_provider_details($id){
		return $this->db->where("status != ",0)->where('id',$id)->get('providers')->row_array();
	}	
	...
	function subscriptionlist($id=''){		
		$this->db->select('SD.*, S.subscription_name, S.fee_description,U.name, U.profile_img,S.fee,S.currency_code');
        $this->db->from('subscription_details SD');
        $this->db->join('subscription_fee S', 'S.id=SD.subscription_id','left');
        $this->db->join('providers U','SD.subscriber_id=U.id','left');
		$this->db->where('S.status = ',1);	  
	...
	function subscriptionlist_filter($user_name,$from,$to){
		if(!empty($from)) {
			$from_date=date("Y-m-d 00:00:00", strtotime($from));
		}else{
			$from_date='';
		...
		$this->db->select('SD.*, S.subscription_name, S.fee_description,U.name, U.profile_img');
		$this->db->from('subscription_details SD');
		$this->db->join('subscription_fee S', 'S.id=SD.subscription_id','left');
		$this->db->join('providers U','SD.subscriber_id=U.id','left');
		...
		if(!empty($from_date)){
			$this->db->where('SD.expiry_date_time >=',$from_date);
		}
		if(!empty($to_date)){
			$this->db->where('SD.expiry_date_time <=',$to_date);
		}
		$this->db->where('S.status = ',1);	  
		$this->db->where('U.status != ',0);
		$this->db->order_by('SD.id','DESC'); 
    ...
    public function pending_service_list($service_title='',$provider='',$users='',$from='',$to='') {
        if(!empty($service_title)) {
            $this->db->where('service_id',$service_title);
        }
