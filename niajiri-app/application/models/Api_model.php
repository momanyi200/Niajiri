...
class Api_model extends CI_Model {
    ...
    public function get_my_service($inputs) {
        ...
        if (count($result) > 0) {
            ...
            foreach ($result as $r) {
                ...
                $offer = $this->get_service_offer(['service_id'=>$r['id'], 'status'=>0, 'df'=>0]);
                // $offer_query=$this->db->last_query();
                $data['service_id'] = $r['id'];
                $data['service_title'] = $r['service_title'];
                ...
                else
                {
                    ...
                    // $data['current_offer$offer_query'] = $offer_query;
                    $data['current_offer'] = '';
                    $data['end_time'] = '';
                }
                $response[] = $data;
        ...
        if (!empty($result)) {
            ...
            if (is_array($get_services) && !empty($get_services)) {
                foreach ($get_services as $key => $c) {
                    ...
                    // $serv_image = $image?$image->service_image:"";
                    $serv_image = array();
    ...
    public function get_bookinglist($provider_id, $status) {
        ...
        if (!empty($status)) {
            ...
            if ((int) $status == 1) {
                $query = $this->db->query("SELECT  `b` . * ,  `s`.`service_title` ,  `s`.`service_image` ,  `s`.`service_amount` ,  `s`.`rating` ,  `s`.`service_image` ,  `c`.`category_name` ,  `sc`.`subcategory_name` ,  `p`.`token` ,`p`.`profile_img` ,  `p`.`mobileno` ,  `p`.`country_code` FROM  `book_service`  `b` LEFT JOIN  `services`  `s` ON  `b`.`service_id` =  `s`.`id` LEFT JOIN  `categories`  `c` ON  `c`.`id` =  `s`.`category` LEFT JOIN  `subcategories`  `sc` ON  `sc`.`id` =  `s`.`subcategory` LEFT JOIN  `users`  `p` ON  `b`.`user_id` =  `p`.`id` WHERE  `b`.`provider_id` =  $provider_id  AND (`b`.`status` = 1  OR  `b`.`status` = 2  OR  `b`.`status` =3 OR `b`.`status` =5 OR `b`.`status` =6 OR `b`.`status` =7 ) AND b.guest_parent_bookid = 0 ");
                $result = $query->result_array();
            }
			if ((int) $status == 2) {
...
/ 2), 2) )) AS distance");
    ...
    public function user_orders_list($limit, $end, $search, $where, $type, $UserCurrency, $currency_sign)
    {
        ...
        WHEN product_cart.delivery_status = 1 THEN "Order Placed" WHEN product_cart.delivery_status = 2 THEN "Order Confirmed" WHEN product_cart.delivery_status = 3 THEN "Shipped" WHEN product_cart.delivery_status = 4 THEN "Out of Delivery" WHEN product_cart.delivery_status = 5 THEN "Delivered & Satisfied " WHEN
            product_cart.delivery_status = 6 THEN "Cancelled By User"
			 WHEN
            product_cart.delivery_status =7 THEN "Cancelled By Provider" 
        ELSE "satisfied"
    ...
    public function provider_orders_list($limit, $end, $search, $where, $type, $UserCurrency, $currency_sign)
    {
        ...
        {
            $this->db->select('product_cart.id, product_cart.order_id, product_cart.shop_id, product_cart.product_id, "'.$currency_sign.'" as currency, "'.$UserCurrency.'" as currency_code, product_cart.product_price, product_cart.qty, product_cart.product_total,product_cart.cart_flag, product_cart.created_at, product_cart.delivery_status, CASE
        WHEN product_cart.delivery_status = 1 THEN "Order Placed" WHEN product_cart.delivery_status = 2 THEN "Order Confirmed" WHEN product_cart.delivery_status = 3 THEN "Shipped" WHEN product_cart.delivery_status = 4 THEN "Out of Delivery" WHEN product_cart.delivery_status = 5 THEN "Delivered" WHEN
            product_cart.delivery_status = 6 THEN "Cancelled By User" WHEN
            product_cart.delivery_status = 8 THEN "Satisfied by user" 
        ELSE "Cancelled By Provider"
    END AS delivery_status_text, 
	CASE
        WHEN (product_cart.delivery_status=8  AND product_cart.cart_flag = 0) THEN "Payment Not Released" 
        WHEN (product_cart.delivery_status=8  AND product_cart.cart_flag = 1) THEN "Payment Released" 
        WHEN (product_cart.delivery_status<5  AND product_cart.cart_flag = 0)  THEN "In Processing" 
        ELSE "" END AS provider_payment,
	product_cart.cancel_reason, products.product_name, CONCAT("'.base_url().'", product_images.product_image) as product_image, product_units.unit_name, shops.shop_name, product_order.order_code, users.name, users.mobileno', false);
        }
  ...
  /* public function featured_category_list($limit, $end, $wcat)
   ...
   // featured_category_list changed on 27-02-23 to filter the category based on lat,lang 
    public function featured_category_list($limit, $end, $wcat,$latitude, $longitude)
    {
        if ($latitude!='' && $longitude!='') {
            $radius = (settingValue('radius'))?settingValue('radius'):'0';
            $this->db->select("id,1.609344 * 3956 * 2 * ASIN(SQRT( POWER(SIN(($latitude  - services.service_latitude) *  pi()/180 / 2), 2) +COS( $latitude  * pi()/180) * COS(services.service_latitude * pi()/180) * POWER(SIN(($longitude  - services.service_longitude) * pi()/180 / 2), 2) )) AS distance");
            $this->db->having('distance <=',$radius);
            $query_service_list =  $this->db->get('services')->result_array();
             if($query_service_list){
                foreach($query_service_list as $key=>$val){
                    $cat_services[] = $val['id'];
                    
                }
                $service_ids = implode(",",$cat_services);
                $service_ids = 'AND s.id in ('.$service_ids.')';
             }
                   
        }
        else { $service_ids =''; } 
        
        $this->db->select('c.id, c.category_name, CONCAT("'.base_url().'", c.category_image) as category_image, (SELECT COUNT(s.id) FROM services AS s WHERE s.category=c.id AND s.status=1 '.$service_ids.') AS service_count');
        $this->db->where($wcat);
        if (isset($limit) && !empty($limit)) {
            $this->db->limit($limit,$end);
        }
       ...
       // $query = $this->db->last_query(); 
         if($query){
            foreach($query as $key=>$val){
                $cat_services_cnt = $this->db->where('category', $val['id'])->count_all_results('services');
                $query[$key]['service_count'] =  $val['service_count'];
            }
        }
        return $query;
    }
    ...
    public function popular_services_list($limit, $page, $where, $search, $whrIn, $latitude, $longitude, $UserCurrency, $currency_sign, $type, $order_by)
    {
        ...
        if ($latitude!='' && $longitude!='') {
          //  $this->db->select("1.609344 * 3956 * 2 * ASIN(SQRT( POWER(SIN(($latitude  - services.service_latitude) *  pi()/180 / 2), 2) +COS( $latitude  * pi()/180) * COS(services.service_latitude * pi()/180) * POWER(SIN(($longitude  - services.service_longitude) * pi()/180 / 2), 2) )) AS distance");
            $this->db->select("1.609344 * 3956 * 2 * ASIN(SQRT( POWER(SIN(($latitude  - services.service_latitude) *  pi()/180 / 2), 3) +COS( $latitude  * pi()/180) * COS(services.service_latitude * pi()/180) * POWER(SIN(($longitude  - services.service_longitude) * pi()/180 / 2), 3) )) AS distance");
    ...
    public function featured_shops_list($limit, $page, $where, $search, $whrIn, $latitude, $longitude, $order_by)
    {
        ...
        $radius = (settingValue('radius'))?settingValue('radius'):'0';
        $this->db->select('categories.category_name,shops.id, shops.shop_name, shops.shop_location, shops.shop_latitude, shops.shop_longitude, CONCAT("'.base_url().'", shops_images.shop_image) as shop_image, providers.name as provider_name, IF(providers.profile_img IS NULL or providers.profile_img = "", "' . $profile_img . '",CONCAT("'.base_url().'", providers.profile_img)) as provider_profile_img, shops.shop_latitude,shops.shop_longitude');
        ...
        if($query){
            ...
            // If Empty List  return some random shop
            $this->db->select('categories.category_name,shops.id, shops.shop_name, shops.shop_location, shops.shop_latitude, shops.shop_longitude, CONCAT("'.base_url().'", shops_images.shop_image) as shop_image, providers.name as provider_name, IF(providers.profile_img IS NULL or providers.profile_img = "", "' . $profile_img . '",CONCAT("'.base_url().'", providers.profile_img)) as provider_profile_img, shops.shop_latitude,shops.shop_longitude');
            $this->db->select('(SELECT COUNT(s.id) FROM services AS s WHERE s.shop_id=shops.id AND s.status=1 ) AS service_count');
            $this->db->select('(SELECT COUNT(p.id) FROM products AS p WHERE p.shop_id=shops.id AND p.status=1 ) AS product_count');
            $this->db->select("1.609344 * 3956 * 2 * ASIN(SQRT( POWER(SIN(($latitude  - shops.shop_latitude) *  pi()/180 / 2), 2) +COS( $latitude  * pi()/180) * COS(shops.shop_latitude * pi()/180) * POWER(SIN(($longitude  - shops.shop_longitude) * pi()/180 / 2), 2) )) AS distance");
    ...
    public function check_current_offer($where_data)
    {
        ...
        $this->db->where($where_data);
        $this->db->where(" start_date <='$cur_date'  AND end_date >='$cur_date'");
        // $this->db->where(" start_time >='$current_time' AND end_time <='$current_time'");
    ...
    public function get_service_offer($where_data)
    {
        $cur_date = date("Y-m-d");
        $current_time = date("H:i");
        $this->db->select('offer_percentage, end_time');
    ...
    public function test_user_product_list($limit, $end, $where, $search, $type, $UserCurrency, $currency_sign)
    {
        if ($type == 'count') 
        {
			...
			$data = array();
			 ...
			 if (count($result) > 0) {				
				foreach ($result as $r) {
					...
					///////// Code Changed on 22-12-2022
					/*if($r['payment_status']==6){
						$statusval = "Payment Completed";
					}*/
					
					if($r['payment_status']==6 && $r['deposit_flag']==0){
						$statusval = "Payment Not Released";
					} 
					if($r['payment_status']==6 && $r['deposit_flag']==1){
						$statusval = "Payment Released";
					} 
					///Code Change End ////////////
    ...
    END AS delivery_status_text, CASE
		...
		$this->db->join('product_images','product_cart.product_id=product_images.product_id and primary_img=1','left');
		$this->db->join('users','product_cart.user_id=users.id','left');
		$this->db->where_in('product_cart.delivery_status', [8,6,7] );	
    ...
    public function mpesa_subscription_pay($inputs) { 
       ...
       $new_details['subscriber_id'] = $mpesa['subscriber_id'] = $user_id;
       $new_details['subscription_id'] = $mpesa['subscription_id'] = $subscription_id;
       $new_details['subscription_date'] = $mpesa['subscription_date'] = $subscription_date;
       $new_details['expiry_date_time'] = $expiry_date_time;
	   ...
	   $mpesa['sub_id'] = $this->db->insert_id();
        ...
        $details_sub = $this->db->get('subscription_details')->row_array();
 			 $mpesa['sub_id'] = $details_sub['id'];
       }
			    $mpesa['tokenid'] =  $tokenid;
				$mpesa['payment_details'] = $payment_details;  
 				$this->db->insert('subscription_payment',$mpesa);
	 ...
	 public function mpesa_mobile_validation($mobile_no){
 			/*	$TransactionType='CustomerBuyGoodsOnline';
				$result=$this->mpesa->checkout($mobile_no,'0',"123456","To check mobile number is MPESA enabled..",$TransactionType);
    ...
    public function get_subscribeFee_info($fee_id) {


        $ret = $this->db->select('subscription_name,fee')->
                        from('subscription_fee')->
	...
	public function get_order_info($order_id) {
        ...
        $ret = $this->db->select('order_code,total_amt,currency_code')->
                        from('product_order')->
	...
	public function delete_images($pid) {
      ...
      $this->db->update('product_images', array('status' => 0,'primary_img' => 0));
        return $this->db->count_all_results('product_images');

    }
    public function delete_images_shop($sid) {
       
    //   $images=$this->db->query("SELECT * FROM shops_images WHERE shop_id=$sid")->result();
