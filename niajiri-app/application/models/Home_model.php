...
class Home_model extends CI_Model
{
    ...
    function get_all_service($params = array(),$inputs=array()){
        ...
        {  
          $location = explode(',', $inputs['user_address']);
	    ...
	    if(empty($inputs['user_address']) && $longitude == '' && $latitude == ''){ 
	     ...
	     if(isset($inputs['category_slug']) && !empty($inputs['category_slug']))
	     {
	     	$this->db->where('c.category_slug',$inputs['category_slug']); 
	     }
	...
	/* New Search - Get All Shops */
	 ...
	 function get_allshops($params = array(),$inputs=array()){ 
        ...
        {    
          $location = explode(',', $inputs['user_address']);
          $this->db->like("s.shop_location",$location[0]);
          // $this->db->like('s.shop_location', $inputs['user_address']);           
        }
	    if(empty($inputs['user_address']) && $longitude == '' && $latitude == '' & $this->session->userdata('current_location')!=""){ 			
        $location = explode(',', $this->session->userdata('current_location'));
        $this->db->like("s.shop_location",$location[0]);
