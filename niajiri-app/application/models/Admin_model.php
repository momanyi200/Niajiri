...
class Admin_model extends CI_Model
{
  ...
  public function update_data($table, $data, $where = []) {
   ...
   public function getSingleData($table,$where=array()) {
    ...
    public function edit_mpesa_gateway($id)
    {
        $query = $this->db->query(" SELECT * FROM `mpesa_payment_gateway` WHERE `id` = $id ");
        $result = $query->row_array();
        return $result;
    }
	...
	public function save_pwdlink_data($user_data)
   {
	...
	public function update_password_resetlink_data($data, $where) {
		$this->db->where($where);
		$status = $this->db->update('forget_password_det', $data);
		return $status;   
    }
