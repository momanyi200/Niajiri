(function($)
{
            ...
            fields: {
                ...
                'products[short_description]': {
                    validators: {         
                        ...
                        stringLength: {
                            max: 500,
                            message: 'The short description must be less than 500 characters'
                        }
                    }
                },
    ...
    $(document).on('click','.edit_billing',function()
    {
        ...
        $.ajax({
            ...
            data:{type:'edit',billing_id:billing_id, csrf_token_name:csrf_token},
            success: function(response)
            {
    ...
    $("#confirm_dbd").on('click', function()
    {
        ...
        $.ajax({
            type: 'POST',
            url: base_url+'user/products/billingdetails',
            data:{type:'delete',billing_id:billing_id, csrf_token_name:csrf_token},
            success: function(response)
            {
    ...
    $("#proceed_payment").on('click', function()
    {
        ...
        {
            toastnotification('error',"Please Select or Add Billing Details",'','','');//save checkout and go to payment
        ...
        else
        {
            if(pay_type == 'paypal') {
                ...
                var url = base_url+'user/products/paypal_order_payment/'+tot_amt+'?orderid='+order_id+'&billing_id='+select_billing+'&address_type='+address_type+'&urlv1='+urlv1;
			...
			else if(pay_type == 'mpesa') {
                var tot_amt = $('#product_total_amt').val();
				$('#mpesa_paymodal1 #amount').val(tot_amt); 
				$('#mpesa_paymodal1 #select_billing').val(select_billing); 
				$('#mpesa_paymodal1 #address_type').val(address_type); 
 				$('#mpesa_paymodal1').modal('show'); 
    ...
    $("#update_order").on('click', function()
    {
        ...
        if(select_billing!=undefined)
        {
            ...
            $.ajax({
                ...
                data:{billing_id:select_billing, order_id:$("#order_id").val(),csrf_token_name:csrf_token},
                success: function(response)
                {
                    console.log(response);
        ...
        {
            toastnotification('error',"Please Select or Add Billing Details",'','','');
        }
    });
    ...
    $(".change_at").on('click', function()
    {
        ...
        var url = base_url+'user/products/moyasar_redirect/'+order_id+'/'+address_type;
    ...
    $(".make_payment").on('click', function()
    {
        ...
        if (gateway == 'paypal' || gateway == 'stripe') 
        {
            ...
            if (vc == true)
            {
                $.ajax({
                    ...
                    data:{order_id:order_id, gateway:gateway, rtype:rtype, csrf_token_name:csrf_token, card_number:card_number, card_name:card_name, expiry_month:expiry_month, expiry_year:expiry_year, cvv:cvv, address_type:address_type},
                    success: function(response)
                    {
                        ...
                        {
                            window.location.href = base_url+'order-confirmation/'+obj.hash_orderid;
                        }
                    }
                });
            }
            ...
            var options = {
                ...
                "handler": function (response){
                    $.ajax({
                        ...
                        type: 'post',
                        dataType: 'json',
                        data: {order_id:order_id,razorpay_payment_id:response.razorpay_payment_id,csrf_token_name:csrf_token,address_type:address_type},
                        success: function (msg) 
                        {                       
                          window.location.href = base_url+'order-confirmation/'+razorpay_redirect;
                        }
                    });
                },
                "theme": {
        ...
        else if (gateway == 'wallet')
        {
            ...
            $.ajax({
                type: 'POST',
                url: base_url+'user/products/make_payment',
                data:{order_id:order_id, gateway:gateway, csrf_token_name:csrf_token, address_type:address_type},
                success: function(response)
                {
                    var obj =  JSON.parse(response);
                    if (obj.error == true) 
    ...
    function add_checkout(product_id, qty)
    {
        $.ajax({
            ...
            success: function(response)
            {
                ...
                {
                    toastnotification('error',"Sorry quantity is not available",'','','');
                }
            }
        });
    }
    function save_billing_details()
    {
        ...
        if(validate == true)
        {
            ...
            $.ajax({
                ...
                data:{id:$("#bd_id").val(), full_name:$("#full_name").val(), phone_no:$("#phone_no").val(), email_id:$("#email_id").val(), address:$("#checkout_address").val(), address_type:address_types, country_id:$("#country_id").val(), state_id:$("#state_id").val(), city_id:$("#city_id").val(), zipcode:$("#zipcode").val(), csrf_token_name:csrf_token},
                success: function(response)
                {
                    $("#billing_list").html(response);
                    $('.my-checkout-col').show();
    ...
    function validate_billing()
    {
        ...
        else if (/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test(email_id) == false) 
        {
            i=1;
    ...
    function validate_card(idg)
    {
        ...
        else if(expiry_month.trim().length<2 || expiry_month.trim().length>2)
        {
            i=1;
    ...
    function add_cart(cart_id, product_id, qty, id_type)
    {
        $.ajax({
            ...
            data:{cart_id:cart_id, product_id:product_id, qty:qty, id_type:id_type, csrf_token_name:csrf_token},
            success: function(response)
            {
                var obj=$.parseJSON(response);
                if(obj.err == false)
                {
                    if(obj.add_rem_type == 'add') {
                        ...
                        toastnotification('success',"Product Removed Successfully",'','','');
                    }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data!=''){
                        ...
                        $('#state_id').append(add);
                        if(selected_id!=''){
                            $('#state_id option[value='+selected_id+']').attr('selected','selected');
                        }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data!=''){
                        ...
                        $('#city_id').append(add);
                        if(selected_id!=''){
                            $('#city_id option[value='+selected_id+']').attr('selected','selected');
                        }
                    }
                }
            });
    ...
    {
        if(type!=''){
            $('#address_types option[value='+type+']').attr('selected','selected');
        }
    }
    //
    //Image Upload
...
var handler = StripeCheckout.configure({
        ...
        $.ajax({
            ...
            data: {order_id: order_id, billing_id: billing_id, amount: order_amount, token: tokenid, address_type: address_type, csrf_token_name:csrf_token},
            type: 'POST',
            dataType: 'JSON',
            success: function(response){
                if(response == 'success') {
                    window.location.href = base_url+'order-confirmation/'+redirect_url;
                } else {
                    window.location.reload();
                }
            },
...
function searchFilter(page_num)
{
    ...
    $.ajax({
        ...
        data:'page='+page_num+'&f_cat_id='+f_cat_id+'&f_pricerange='+f_pricerange+'&f_cc='+f_cc+'&csrf_token_name='+csrf_token,
        beforeSend: function(){
            $('.loading').show();
        },
...
function orderFilter(page_num)
{
    ...
    $.ajax({
        ...
        data:'page='+page_num+'&order_code='+$("#order_code").val()+'&csrf_token_name='+csrf_token+'&shop_name='+$("#shop_name").val()+'&product_name='+$("#product_name").val()+'&delivery_status='+$("#delivery_status").val(),
        beforeSend: function(){
            $('.loading').show();
        },
        success: function(html)
...
function checkordercancel(cart_id, order_id)
{
    $.ajax({
        ...
        data:{'submit_type':'check', cart_id:cart_id, order_id:order_id, csrf_token_name:csrf_token},
        success: function(response)
        {
            var obj=$.parseJSON(response);
            if (obj.error == true) 
...
function cancel_order()
{
    ...
    if (cancel_reason!='') {
        $.ajax({
            ...
            data:{cart_id:$("#hc_id").val(), order_id:$("#ho_id").val(), csrf_token_name:csrf_token, 'cancel_reason':cancel_reason},
            success: function(response)
            {
...
function satisfy_order(cart_id, order_id)
{
  // alert(cart_id);
    $.ajax({
        type: 'POST',
        url: base_url+'user/products/changeSatisfy',
        data:{'submit_type':'check', cart_id:cart_id, order_id:order_id, csrf_token_name:csrf_token},
        success: function(response)
        {
            var obj=$.parseJSON(response);
            ...
            {
                 swal({                  
                    title: 'Satisfaction on Product Delivery',
                    text: obj.msg,                  
                    icon: "success",
                    button: "okay",
                    closeOnEsc: false,
...
function porderFilter(page_num)
{
    ...
    $.ajax({
        ...
        data:'page='+page_num+'&order_code='+$("#order_code").val()+'&csrf_token_name='+csrf_token+'&user_name='+$("#user_name").val()+'&product_name='+$("#product_name").val()+'&delivery_status='+$("#delivery_status").val(),
        beforeSend: function(){
            $('.loading').show();
        },
        success: function(html)
...
function confirm_product_delete()
{
    $.ajax({
        ...
        success: function(response)
        {
            ...
            $("#prow_"+$("#hp_did").val()).fadeOut(300, function(){ $(this).remove();});
...
function getData(page_num)
{
    ...
    $.ajax({
        ...
        data:'page='+page_num+'&csrf_token_name='+csrf_token+'&shop_id='+$("#hshop_id").val(),
        beforeSend: function(){
            $('.loading').show();
        },
        success: function(html)
...
function empty_form() {
	...
	$("#mpesa_prd_chkout_submit").on("click", function(event){
		event.preventDefault();
			var order_id = $("#order_id").val();
 			var mob_no = $("#mob_no").val(); 
			var amount = $("#amount").val();
			var billing_id= $("#select_billing").val();
			var address_type= $("#address_type").val();
				$.ajax({
				 url: base_url+'user/products/buyProduct_mpesa_payment/',
				 data: {type:"M-PESA",order_id: order_id, amount: amount, csrf_token_name:csrf_token,mobile_no: mob_no,billing_id: billing_id,address_type:address_type },
				 type: 'POST',					
				 success: function(response){ 
					if(response!=''){
