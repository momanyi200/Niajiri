	...
	if (n == 0) {
		...
		$('form[name="add_shops"]').submit(function(e){
			...
			if(mobileno.length == 0){
				$(".mobilenoerr").text('Please Enter Mobile Number');
				n=1;
			}else{
				$(".mobilenoerr").text(mobileno);
			}
...
$('#shopsubmit').on("click", function(){	
	...
	var about = $('#about').val();
	var shop_location = $('#shop_location').val();
	var images = $('#images').val()?$('#images').val():$(".loaded_img").length;
	shop_title = $.trim(shop_title);
	if(shop_title.length == 0){
		$(".business_title").text('Please Enter Shop Title');
		n=1;		
...
// 	}
    ...
    if (category == '' || category == null) {
        $('.dropdown button[data-id="category"]').css({"border": "1px solid red"});
        $(".categoryerr").text('Please Select Category');
        n2 = 2;
    } else {
        $('.dropdown button[data-id="category"]').css({"border": ""	});
        $(".categoryerr").text('');
    }
	var subcategory = $("#subcategory").val();
    if (subcategory == '' || subcategory == null ) {
       $(".subcategoryerr").text('Please Select Subcategory');
        $('.dropdown button[data-id="subcategory"]').css({"border": "1px solid red"});
        n2 = 2;
    } else {
       $(".subcategoryerr").text('');
        $('.dropdown button[data-id="subcategory"]').css({"border": ""	});
    }
