(function($) {
	...
	$( document ).ready(function() {
		$('#history_page').hide();
		$('#home_page').show();
		var tok = $(".history_append_fun:first-child").attr('data-token');
		history_append_fun(tok);
                var refreshChatHistory = setInterval(function(){history_append_fun(tok);}, 3000);

		$('.history_append_fun').on('click',function(){
			var id=$(this).attr('data-token');
			history_append_fun(id);
			clearInterval(refreshChatHistory);

			 setInterval(function(){history_append_fun(id);}, 300);
		});  
    ...
    success:function(data){
     ...
     $.ajax({
       ...
       success:function(fetch){
        ...
        if(Data.self_info != null && Data.partner_info != null) {
