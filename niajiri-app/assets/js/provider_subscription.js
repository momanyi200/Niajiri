(function($) {
	...
	function callStripe(e) {
		...
		if(parseInt(final_gig_amount)==0.00) {
			...
			if (payment_type == "mpesa" && payment_type != undefined) {
 				
				$('#mpesa_paymodal1 #amount').val(final_gig_amount);
				$('#mpesa_paymodal1 #sub_id').val(sub_id);
...
$('#mpesa_sub_chkout_submit').on('click', function(e) {
		e.preventDefault();										     
		var sub_id = $('#sub_id').val();
		var amount = Math.round($('#amount').val());
		var final_gig_currency = $('#user_currency').val();
		var mob_no = $('#mob_no').val();
		//alert(base_url+'user/subscription/mpesa_subscription_payment');
		
		 $.ajax({
							url: base_url+'user/subscription/mpesa_subscription_payment',
							type: 'POST',
							data: {sub_id:sub_id, amount:amount,mobile_no:mob_no,csrf_token_name:csrf_token},
							success: function (response) {	
							alert(response);
							   check_commercial_reg_status();						
							}
