...
function get_services() { 
   ...
   var category_slug = ''; var subcategory_slug = '';
   var categories = ''; var subcategories = '';
   categories=$('#categories').val();
   
   if(str[3]=="search")
    {
        var c_slug=str[4].includes("#");
        if(str[4].includes("#")==true){
            var c_slug=str[4].split("#");
            category_slug=c_slug[0];
        }
        else{
            category_slug=str[4];
        }
...
'</div>');
$('#dataList').empty();
   $.post(base_url+'home/all_services',{min_price:min_price,max_price:max_price,sort_by:sort_by,common_search:common_search,categories:categories,category_slug:category_slug,subcategory_slug:subcategory_slug,subcategories:subcategories,service_latitude:service_latitude,service_longitude:service_longitude,csrf_token_name:csrf_token,user_address:user_address,sub_subcategories:sub_subcategories},function(data){
